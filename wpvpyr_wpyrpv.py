...
wpyrpv = np.linspace(0.005, 0.06, grid_size)
wpvpyr = np.linspace(-0.08, -0.005, grid_size)
p_space = np.meshgrid(wpvpyr, wpyrpv)
...
# for i in tqdm(range(len(wpyrpv))):
#     for j in range(len(wpvpyr)):
#         new_cell._set_connections() # reset connections
#         # set parameters
#         new_cell.wpvpyr = wpvpyr[j] 
#         new_cell.wpyrpv = wpyrpv[i]
...
x_label = '$w_{PV \\rightarrow PYR}$'
y_label = '$w_{PYR \\rightarrow PV}$'
