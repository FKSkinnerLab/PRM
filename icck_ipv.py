...
i_cck = np.linspace(0.05, 1.5, grid_size) #default range = [0.025, 1.5]
i_pv = np.linspace(0.05, 1.5, grid_size) #default range = [0.05, 0.8]
p_space = np.meshgrid(i_cck, i_pv)
...
for i in tqdm(range(len(i_pv))):
    for j in range(len(i_cck)):
        new_cell._set_connections() # reset connections
        # set parameters
        new_cell.i_cck = i_cck[j] 
        new_cell.wpyrpyr = i_pv[i]
...
# save data 
# add path to directory you want to save results to
np.savez_compressed(f'./sim_data/{px}_{py}', tp=theta_power, gp=gamma_power, tf=theta_freq, gf=gamma_freq)

# #if using saved data
# loaded_file = np.load(f'./sim_data/{px}_{py}_eqr.npz')
# theta_power = loaded_file['tp']
# theta_freq = loaded_file['tf']
# gamma_power = loaded_file['gp']
# gamma_freq = loaded_file['gf']
...
dir = f'./Figures/NEW_PRM/{px}_{py}/' # add path to the directory you want to save the images to
...
# Theta Power
for i in range(4):    
    fig, ax = plt.subplots(dpi=400, figsize=[9,6.3])
    plt.grid(False)
