...
px = 'wbicpyr'; py = 'wpyrbic'

wbicpyr = np.linspace(-0.075, -0.005, grid_size)
wpyrbic = np.linspace(0.03, 0.08, grid_size)
p_space = np.meshgrid(wbicpyr, wpyrbic)
...
# for i in tqdm(range(len(wpyrbic))):
#     for j in range(len(wbicpyr)):
#         new_cell._set_connections() # reset connections
#         # set parameters
#         new_cell.wbicpyr = wbicpyr[j] 
#         new_cell.wpyrbic = wpyrbic[i]
...
#if using saved data
loaded_file = np.load(f'./sim_data/{px}_{py}.npz')
...
x_label = '$w_{BiC \\rightarrow PYR}$'
y_label = '$w_{PYR \\rightarrow BiC}$'
