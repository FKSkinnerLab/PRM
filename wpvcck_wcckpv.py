...
# wcckpv = np.linspace(-0.3, -0.075, grid_size)
# wpvcck = np.linspace(-0.15, -0.02, grid_size)
# equal ranges
wcckpv = np.linspace(-0.3, -0.02, grid_size)
wpvcck = np.linspace(-0.3, -0.02, grid_size)
p_space = np.meshgrid(wpvcck, wcckpv)
...
# for i in tqdm(range(len(wcckpv))):
#     for j in range(len(wpvcck)):
#         new_cell._set_connections() # reset connections
#         # set parameters
#         new_cell.wpvcck = wpvcck[j] 
#         new_cell.wcckpv = wcckpv[i]
...
# np.savez_compressed(f'./sim_data/{px}_{py}.npz', tp=theta_power, gp=gamma_power, tf=theta_freq, gf=gamma_freq)
...
dir = f'./Figures/NEW_PRM/{px}_{py}/eqr'
...
x_label = '$w_{PV \\rightarrow CCK}$'
y_label = '$w_{CCK \\rightarrow PV}$'
